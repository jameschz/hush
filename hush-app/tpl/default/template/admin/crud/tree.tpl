...
<table class="tlist" >
	...
	<tbody>
		...
		<tr onclick="javascript:$('.menu{$node.id}').toggle();">
			...
			<td align="left"><a href="javascript:openWindow('{$aps.action}/a/edit?id={$node.id}','修改{$title}');"><u>{$node.name}</u></a><img src="{$_root}img/sort_asc.gif" /></td>
			<td align="left">{$node.code}</td>
			<td align="right">
				<a href="javascript:openWindow('{$aps.action}/a/edit?id={$node.id}','修改{$title}');">编辑</a> | 
			...
			<tr class="menu{$node.id}">
				...
				<td align="left">└ <a href="javascript:openWindow('{$aps.action}/a/edit?id={$node1.id}','修改{$title}');"><u>{$node1.name}</u></a></td>
				<td align="left">{$node1.code}</td>
				<td align="right">
					<a href="javascript:openWindow('{$aps.action}/a/edit?id={$node1.id}','修改{$title}');">编辑</a> | 
	...
	<tfoot>
		<tr>
			<td colspan="5">
				<button type="button" class="btn1s" onclick="javascript:openWindow('{$aps.action}/a/add');">新增{$title}</button>
			</td>
		</tr>
	</tfoot>
</table>
